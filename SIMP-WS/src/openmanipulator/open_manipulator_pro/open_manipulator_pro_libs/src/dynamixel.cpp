...
*****************************************************************************/
/**
 * IRVIN: Wrapper function that initializes JointDynamixe (Dynamixel Servo)
 */
...
/**
 * Wrapper function to send a joint actuator value / "write a goal position"
 * Writes a Goal Position as Radians
 */
...
bool JointDynamixel::initialize(std::vector<uint8_t> actuator_id, STRING dxl_device_name, STRING dxl_baud_rate)
{
  ...
  for (uint8_t index = 0; index < dynamixel_.num; index++)
  {
    ...
    printf("JOINTDYNAMIXEL:::: PING Getting Model Number ID: %d\n", id);
    result = dynamixel_workbench_->ping(id, &get_model_number, &log);
    printf("JOINTDYNAMIXEL:::: RESULT Model Number: %d\n", get_model_number);
    if (result == false)
    {
      log::error(log);
    ...
    {
      char str[100];
      sprintf(str, "Joint Dynamixel ID : %d, Model Name : %s\n", id, dynamixel_workbench_->getModelName(id));
      log::println(str);
      ...
      // IRVIN: This sets the a return_delay_time to the motor
      printf("JOINTDYNAMIXEL::::: Setting return_delay_time ID: %d id,  COMMAND: %s, VALUE:  50, %s\n", id, return_delay_time_char, &log);
      // result = dynamixel_workbench_->writeRegister(id, return_delay_time_char, 0, &log);
      // IRVIN Modified to increase return delay time to reduce packet error 
      result = dynamixel_workbench_->writeRegister(id, return_delay_time_char, 50, &log);
      if (result == false)
      {
        log::error(log);
...
/**
 * Writes a Goal Position to all actuators in a Vector
 * Converts Radian values to Position values
 */
